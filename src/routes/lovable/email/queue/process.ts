...
// Async local transport reuse. NodeMailer verifies on first send; we keep a single
// shared transporter across the batch instead of rebuilding it per message.
...
// Check if an error is a rate-limit (429) response from an upstream provider.
// NodeMailer surfaces these wrapped in Error.message; SMTP servers may use 450/451.
function isRateLimited(error: unknown): boolean {
  ...
  const status =
    error && typeof error === "object" && "status" in error
      ...
      : undefined;
  return status === 429 || /(429|451|450)/.test(msg);
}

// Check for permanent auth/config failures. Retrying won't help — move to DLQ.
export function isForbidden(error: unknown): boolean {
  ...
  const status =
    error && typeof error === "object" && "status" in error
      ? (error as { status: number }).status
      : undefined;
  return status === 403 || /(535|553|5\.7\.|authentication failed|bad credentials)/i.test(msg);
}

// Extract Retry-After seconds from a structured upstream error, or default to 60s.
        ...
        for (const queue of ["auth_emails", "transactional_emails"]) {
          ...
          for (let i = 0; i < messages.length; i++) {
            ...
            try {
              ...
              // Permanent auth/config failures: move straight to DLQ and stop the batch.
